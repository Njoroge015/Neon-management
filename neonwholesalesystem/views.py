...
def create_bill(request):
    ...
    custnumb = request.POST.get('custmobile', '').strip()
    custemail = request.POST.get('custemail', '').strip()
    payment = request.POST.get('payment')
    discount = float(request.POST.get('discount') or 0)
    percent_discount = float(request.POST.get('discount_percentage') or 0)
    payprice = int(totalprice['total'] - discount)
    if not custname or not custnumb:
        messages.warning(request, 'Enter the Customer Details.')
        return redirect('/')
    if len(custnumb) != 10 or not custnumb.isdigit():
    ...
    if custemail:
        ...
        try:
            ...
            messages.warning(request, 'Bill created, but the email could not be sent.')
